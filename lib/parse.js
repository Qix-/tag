...
const keywords = {
	ECHO: async (ctx, st) => await ctx.call('tag', 'echo', await parseArgs(ctx, st.arguments), st.location),

	ERROR: async (ctx, st) => await ctx.call('tag', 'error', await parseArgs(ctx, st.arguments), st.location),
