...
class Context {
	...
	async use(name, location) {
		...
		const resolved = await (() => {
			const fullpath = parseArgs(this, tagpath.value);
			...
			if (fullpath.length > 1) {
				throw new TagError({location}, 'TAGPATH has multiple arguments; use a quoted string for paths containing spaces');
			}

			try {
				return resolvePath(name, fullpath[0].trim(), tries);
			} catch (error) {
