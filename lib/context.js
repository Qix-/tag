...
class Context {
	...
	unset(name) {
		...
		if (this.localNamespace) {
			delete this.localNamespace[name];
		}
	}
...
const keywords = {
	...
	ON: (ctx, st) => {
		...
		if (tags.length === 0) {
			throw new TagError(st, `ON keyword requires one or more tags to enable`);
		}
	...
	OFF: (ctx, st) => {
		...
		if (tags.length === 0) {
			throw new TagError(st, `OFF keyword requires one or more tags to disable`);
		}
	...
	UNSET: (ctx, st) => {
		const args = parseArgs(ctx, st.arguments);
		if (args.length === 0) {
			throw new TagError(st, 'UNSET keyword requires at least one or more variable names');
		}
